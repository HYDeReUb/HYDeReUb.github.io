...
while true; do
 ...
 read -n 1 -p "請選擇代號:" ch
    case $ch in
        ...
        [Dd]* ) echo "" && echo "" && echo "終端機貼上方式:[Ctrl]+[Shift]+[V]" && echo "下載位置: 家目錄/youtube下載" && read -p "請輸入網址:" url && read -p "請輸入檔名(不要輸入副檔名):" fname && normal='~/youtube下載/' && youtube-dl -o $normal$fname $url && echo "" && read -p "完成，按[Enter]結束下載" && clear;;
        [Qq]* ) exit;;
