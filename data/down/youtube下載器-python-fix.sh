...
while true; do
 ...
 read -n 1 -p "請選擇代號:" ch
    case $ch in
        ...
        [Uu]* ) echo "" && echo "" && sudo wget https://yt-dl.org/downloads/latest/youtube-dl -O /usr/local/bin/youtube-dl  &&  sudo chmod a+rx /usr/local/bin/youtube-dl && python3 $(which youtube-dl) -U && echo "" && read -p "完成，按[Enter]結束更新" && clear;;
